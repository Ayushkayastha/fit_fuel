...
class AfternoonSnack extends ConsumerStatefulWidget {
  const AfternoonSnack({super.key});

  @override
  ConsumerState<AfternoonSnack> createState() => _AfternoonSnackState();
}

class _AfternoonSnackState extends ConsumerState<AfternoonSnack> {
  ...
  Widget build(BuildContext context) {
    ...
    final AfternoonSnackItems = dateLog?.afternoonSnack ?? [];
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('AfternoonSnack'),
        centerTitle: true,
      ...
      body: Column(
        children: [
                  ...
                  return ListTile(
                      ...
                      onTap: () {
                        context.pushNamed(
                          ...
                          extra: FoodDetailArgs(food: food, mealType: 'afternoonSnack'),
                        ).then((_) {
                          ref.read(showFoodListProvider.notifier).state = false;
                          _focusNode.unfocus();
                        });
          ...
          if (!showList)
            Expanded(
              child: AfternoonSnackItems.isEmpty
                  ? const Center(child: Text("No AfternoonSnack logged for this date."))
                  : ListView.builder(
                itemCount: AfternoonSnackItems.length,
                itemBuilder: (context, index) {
                  final item = AfternoonSnackItems[index];
                  return ListTile(
