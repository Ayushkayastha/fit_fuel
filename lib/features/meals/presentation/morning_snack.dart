...
class _MoriningSnackState extends ConsumerState<MoriningSnack> {
  ...
  Widget build(BuildContext context) {
    ...
    final MoriningSnackItems = dateLog?.morningSnack ?? [];
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          if (!showList)
            Expanded(
              child: MoriningSnackItems.isEmpty
                  ? const Center(child: Text("No MoriningSnack logged for this date."))
                  : ListView.builder(
                itemCount: MoriningSnackItems.length,
                itemBuilder: (context, index) {
                  final item = MoriningSnackItems[index];
                  return ListTile(
